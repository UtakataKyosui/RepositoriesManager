...
#### 通常検索
検索クエリを入力すると、以下の項目で部分一致検索が行われます：
- リポジトリ名
- 説明文
- トピック（タグ）

例：`rust` と入力すると、名前、説明、トピックのいずれかに「rust」を含むリポジトリが表示されます。

#### トピック検索
`topics:`をプレフィックスとして付けると、トピックのみで検索できます：
- `topics:rust` - rustトピックを持つリポジトリのみ表示
- `topics:rust,cli` - rustとcliの両方のトピックを持つリポジトリのみ表示（カンマ区切りでAND検索）
